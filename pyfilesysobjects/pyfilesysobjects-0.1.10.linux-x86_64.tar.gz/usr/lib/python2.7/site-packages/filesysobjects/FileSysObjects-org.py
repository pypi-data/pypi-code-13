...
def findRelPathInSearchPath(spath, plist=None, **kargs):
    ...
    suitable to 'spath'. The path items are treated as entry point for specific 
    extension branches.
...
def splitPathVar(pathvar, **kargs):
    ...
    Args:
        ...
        **kargs: 
            passed through options for  each, refer to 'splitAppPrefix'
            `[see] <#splitappprefix>`_.


    Returns:
        When split successful returns list of tuples containing:
          ::
        ...
        For details refer to 'splitAppPrefix' `[see] <#splitappprefix>`_.
    ...
    for i in _CSPLITPATH.finditer(pathvar):
        for g in _CSPLITPATHg: # do this for getting the syntax term
            ...
            if g == 92: # shortcut for empty group == empty path entry 
                if ias:
                    ret.append(('IAS', '', '', ''))
                elif raw: # prio higher than rtype
                    ret.append(('', '', '', ''))
                elif rtype:
                    ret.append(('', '', '', ''))
                else:
            ...
            if ias: # ignore application separator
                ...
                if raw:
                    ...
                    if s > 0 and pathvar[s-1] == os.sep:
                        ret.append((i.group(g), '', '', pathvar[s-1:e]))
                    else:
                        ret.append((i.group(g), '', '', pathvar[s:e]))
                    break
            ...
            if i.group(g + 3) or i.group(g):  # 4:local file system / 0:uri or IEEE/UNC/SMB
                ...
                if raw:
                    ret.append((pathvar[i.start(g):i.start(g+1)], pathvar[i.start(g+1):i.start(g+2)], pathvar[i.start(g+2):i.start(g+3)], ''))
                elif rtype:
